...
export default function ServiceNeighborhoodPage({ params }: { params: { service: string; neighborhood: string } }) {
  ...
  const localFaqs = [
    ...
    { q: `Which contractors do ${s.name.toLowerCase()} in ${n.name}?`, a: `Our platform vets contractors across the ${n.area} area. All hold a valid Oregon CCB licence and have passed our 47-point review. Submit your details to get matched within 48 hours.` },
  ]
  ...
  return (
    <>
      ...
      {/* ── NEIGHBOURHOOD SWITCHER — top of page ── */}
      ...
      <div className="grid-nbhd-hero section-pad" style={{ background: 'var(--bg2)' }}>
        <div>
          ...
          <p style={{ ...f, fontSize: 'clamp(1rem,2.5vw,1.05rem)', color: 'var(--muted)', maxWidth: '560px', lineHeight: 1.75, fontWeight: 300, marginBottom: '2.5rem' }}>
            {inject(s.intro, n, s)}
          </p>
      ...
      <section className="section-pad" style={{ background: 'var(--bg)' }}>
        <div className="content-wrap">
          ...
          <p style={{ ...f, fontSize: '1rem', color: 'var(--text)', lineHeight: 1.75, fontWeight: 300, marginBottom: '1.5rem' }}>
      ...
      <section className="section-pad" style={{ background: 'var(--bg2)' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          {services.filter(x => x.slug !== s.slug).map(x => (
            <Link key={x.slug} href={`/${x.slug}/${n.slug}`} style={{ ...c, fontSize: '0.85rem', letterSpacing: '0.06em', color: 'var(--amber)', padding: '0.5rem 1rem', border: '1px solid var(--bdr)', textDecoration: 'none', background: 'var(--bg)', whiteSpace: 'nowrap' }}>
              {x.name} →
            </Link>
          ))}
          <Link href={`/portland/${n.slug}`} style={{ ...c, fontSize: '0.85rem', letterSpacing: '0.06em', color: 'var(--muted)', padding: '0.5rem 1rem', border: '1px solid var(--bdr)', textDecoration: 'none', background: 'var(--bg)', whiteSpace: 'nowrap' }}>
            {n.name} Overview →
          </Link>
        </div>
      </section>
