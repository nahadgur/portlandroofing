...
const urgencies = [
  { id: 'leak',   label: 'Leaking Now',       sub: 'Emergency repair needed',  icon: '⚡' },
...
export default function LeadForm() {
  ...
  if (submitted) {
    return (
      <div style={{ padding: '3rem 2rem', background: 'var(--bg2)', textAlign: 'center', borderTop: '3px solid var(--green)' }}>
        ...
        <div style={{ padding: '1rem', background: 'var(--bg3)', border: '1px solid var(--bdr)', display: 'inline-block', textAlign: 'left', minWidth: '240px' }}>
  ...
  const nextBtn = (enabled: boolean): React.CSSProperties => ({
    ...
    padding: '1.1rem',
    border: 'none',
    background: enabled ? 'var(--amber)' : '#2A3140',
    color: enabled ? '#000' : '#4A5568',
    fontFamily: 'var(--font-barlow-cond)',
    fontWeight: 700,
    fontSize: '1.05rem',
  ...
  return (
    <div style={{ background: 'var(--bg2)', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* ── TOP ACCENT + HEADER ── */}
      <div style={{ borderTop: '3px solid var(--amber)' }}>
        <div style={{ padding: '1.8rem 2rem 0' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-bebas)', fontSize: 'clamp(1.8rem,3vw,2.4rem)', color: 'var(--text)', lineHeight: 0.95, marginBottom: '0.5rem', letterSpacing: '0.02em' }}>
            GET YOUR<br />
            <span style={{ color: 'var(--amber)' }}>FREE QUOTES</span>
          </h2>
          ...
          <div style={{ display: 'flex', gap: '0', marginBottom: '1.8rem', borderBottom: '1px solid var(--bdr)' }}>
            {steps.map(({ n, label }) => (
              <div key={n} style={{
                flex: 1, paddingBottom: '0.7rem',
                borderBottom: `2px solid ${step === n ? 'var(--amber)' : step > n ? 'var(--green)' : 'transparent'}`,
                marginBottom: '-1px',
              }}>
        ...
        {step === 1 && (
          <div>
            ...
            <input
              ...
              onBlur={e => e.currentTarget.style.borderColor = data.zip.length >= 5 ? 'var(--amber)' : '#3A4250'}
              autoFocus
            />
            ...
            <button
              ...
              style={nextBtn(data.zip.length >= 5)}
            >
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem' }}>
              {materials.map(m => (
                <button
                  key={m.id}
                  onClick={() => set('material', m.label)}
                  style={data.material === m.label ? optionSel : optionBase}
                >
                  <div style={{ fontWeight: 700, marginBottom: '0.15rem' }}>{m.label}</div>
                  <div style={{ fontSize: '0.7rem', color: data.material === m.label ? 'rgba(245,166,35,0.7)' : 'var(--muted)', fontFamily: 'var(--font-barlow)' }}>{m.sub}</div>
                </button>
              ))}
            </div>
            <div style={{ display: 'flex', gap: '0.6rem' }}>
              ...
              <button
                ...
                style={{ ...nextBtn(!!data.material), flex: 1 }}
              >
                NEXT: SET TIMELINE →
              </button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem' }}>
              {urgencies.map(u => (
                <button
                  ...
                  style={data.urgency === u.label ? optionSel : optionBase}
                >
                  <div style={{ fontSize: '1.1rem', marginBottom: '0.3rem' }}>{u.icon}</div>
                  <div style={{ fontWeight: 700, marginBottom: '0.1rem' }}>{u.label}</div>
                  <div style={{ fontSize: '0.68rem', color: data.urgency === u.label ? 'rgba(245,166,35,0.7)' : 'var(--muted)', fontFamily: 'var(--font-barlow)' }}>{u.sub}</div>
                </button>
              ))}
            </div>
            <div style={{ display: 'flex', gap: '0.6rem' }}>
              ...
              <button
                ...
                style={{ ...nextBtn(!!data.urgency), flex: 1 }}
              >
                LAST STEP →
              </button>
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', marginBottom: '0.2rem' }}>
              <input
                ...
                style={inputStyle}
                onFocus={e => e.currentTarget.style.borderColor = 'var(--amber)'}
                onBlur={e => e.currentTarget.style.borderColor = data.name ? 'var(--amber)' : '#3A4250'}
              />
              <input
                type="tel"
                ...
                style={inputStyle}
                onFocus={e => e.currentTarget.style.borderColor = 'var(--amber)'}
                onBlur={e => e.currentTarget.style.borderColor = data.phone ? 'var(--amber)' : '#3A4250'}
              />
              <input
                type="email"
                ...
                style={inputStyle}
                onFocus={e => e.currentTarget.style.borderColor = 'var(--amber)'}
                onBlur={e => e.currentTarget.style.borderColor = '#3A4250'}
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.6rem' }}>
              ...
              <button
                ...
                style={{ ...nextBtn(!submitting && !!(data.name && data.phone)), flex: 1 }}
              >
      ...
      {/* ── TRUST FOOTER ── */}
      <div style={{ borderTop: '1px solid var(--bdr)', padding: '1rem 2rem', display: 'flex', gap: '1.2rem', flexWrap: 'wrap', background: 'rgba(0,0,0,0.2)' }}>
        {[
