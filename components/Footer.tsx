...
const footerLinks = {
  ...
  Resources: [
    ...
    { label: 'Permits by Neighborhood', href: '/guides/portland-roofing-permits-guide' },
    ...
    <footer style={{ background: 'var(--bg2)', padding: '3rem', borderTop: '1px solid var(--bdr)' }}>
      <div className="grid-footer-top">
        <div>
          ...
          <p style={{ fontFamily: 'var(--font-barlow)', fontSize: '0.88rem', color: 'var(--muted)', lineHeight: 1.7, fontWeight: 300, maxWidth: '300px' }}>
          ...
          <a href={`tel:${SITE.phone.replace(/\D/g, '')}`} style={{ display: 'block', marginTop: '1.2rem', fontFamily: 'var(--font-space-mono)', fontSize: '0.8rem', color: 'var(--amber)', textDecoration: 'none' }}>
            {SITE.phone}
          </a>
        </div>
        ...
        {Object.entries(footerLinks).map(([heading, links]) => (
          <div key={heading}>
            <h4 style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.65rem', color: 'var(--amber)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '1rem' }}>
              {heading}
            </h4>
            <ul style={{ listStyle: 'none' }}>
              {links.map(({ label, href }) => (
                <li key={href} style={{ marginBottom: '0.6rem' }}>
                  <Link href={href} style={{ fontFamily: 'var(--font-barlow)', fontSize: '0.88rem', color: 'var(--muted)', textDecoration: 'none' }}>
                    {label}
                  </Link>
                </li>
              ))}
      ...
      <div style={{ paddingTop: '2rem', borderTop: '1px solid var(--bdr)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
        <p style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.68rem', color: 'var(--muted)' }}>
          © {year} {SITE.name} · {SITE.domain} · All rights reserved.
        </p>
        <p style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.68rem', color: 'var(--muted)' }}>
